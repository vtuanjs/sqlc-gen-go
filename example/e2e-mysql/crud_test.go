...
func TestUserCRUD(t *testing.T) {
	...
	// :execlastid returns the AUTO_INCREMENT id MySQL assigned.
	id, err := q.CreateUser(ctx, dbmysql.CreateUserParams{
		Name:  "crud",
	...
	if id == 0 {
		t.Fatal("CreateUser: got last insert id 0")
	}
	...
	t.Run("UpdateUser", func(t *testing.T) {
		err := q.UpdateUser(ctx, dbmysql.UpdateUserParams{
			Name:  "crud2",
		...
		if u.Name != "crud2" || u.Email != "crud2@example.com" {
			t.Errorf("got %+v, want the updated name/email", u)
		}
	})
	...
	t.Run("ListUsers_SqlcSlice", func(t *testing.T) {
		...
		// ORDER BY name
		if users[0].ID != id || users[1].ID != other.ID {
			t.Errorf("got ids [%d %d], want [%d %d]", users[0].ID, users[1].ID, id, other.ID)
		}
	})
...
func TestOrderQueries(t *testing.T) {
	...
	orderID, err := q.CreateOrder(ctx, dbmysql.CreateOrderParams{
		UserID: user.ID,
		Amount: "19.99",
		Status: "pending",
	})
	...
	t.Cleanup(func() { db.Exec("DELETE FROM orders WHERE id = ?", orderID) })

	// A second, older order — inserted here (not in a subtest) so it outlives
	// the subtest that first observes it.
	...
	t.Run("UpdateOrderStatus", func(t *testing.T) {
		...
		n, err := q.UpdateOrderStatus(ctx, dbmysql.UpdateOrderStatusParams{
			Status: "shipped",
			ID:     orderID,
		})
		if err != nil {
	...
	t.Run("GetUserOrderSummary", func(t *testing.T) {
		...
		// interface{} — the driver hands back the raw DECIMAL bytes.
		if got := string(summary.TotalSpent.([]byte)); got != "20.99" {
...
func TestProductQueries(t *testing.T) {
	...
	t.Cleanup(func() { db.Exec("DELETE FROM products") })

	id, err := q.CreateProduct(ctx, dbmysql.CreateProductParams{
		Name:  sql.NullString{String: "widget", Valid: true},
		Price: "9.99",
		Stock: sql.NullInt32{Int32: 5, Valid: true},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	nullID, err := q.CreateProduct(ctx, dbmysql.CreateProductParams{Price: "1.00"})
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("GetProduct", func(t *testing.T) {
		...
		if p.Price != "9.99" {
			t.Errorf("got price %q, want 9.99", p.Price)
		}
	})

	t.Run("NullableColumns", func(t *testing.T) {
		p, err := q.GetProduct(ctx, nullID)
		if err != nil {
			t.Fatal(err)
		}
		if p.Name.Valid {
	...
	t.Run("GetProductPrice", func(t *testing.T) {
		price, err := q.GetProductPrice(ctx, id)
		if err != nil {
			t.Fatal(err)
		}
		if price != "9.99" {
			t.Errorf("got %q, want 9.99", price)
		}
	})

	t.Run("UpdateProductStock", func(t *testing.T) {
		err := q.UpdateProductStock(ctx, dbmysql.UpdateProductStockParams{
			Stock: sql.NullInt32{Int32: 42, Valid: true},
			ID:    id,
		})
		if err != nil {
			t.Fatal(err)
	...
	t.Run("DeleteProduct", func(t *testing.T) {
		if err := q.DeleteProduct(ctx, nullID); err != nil {
			t.Fatal(err)
		}
		if _, err := q.GetProduct(ctx, nullID); !errors.Is(err, sql.ErrNoRows) {
			t.Errorf("got err %v, want sql.ErrNoRows after delete", err)
		}
	})
}
